...
namespace AlmostEngine.Preview
{
	public class ResolutionSettingsWindow : EditorWindow
	{
		...
		{
			PreviewConfigAsset asset = AssetUtils.GetOrCreate<PreviewConfigAsset> ("Assets/UniversalDevicePreviewConfig.asset");
		...
		protected void DrawGallery ()
		{
			...
			if (m_ConfigAsset.m_DrawSafeArea) {
				EditorGUILayout.HelpBox ("Note that safe area is only displayed with devices containing safe area data. " +
					"If you get more safe area values, please send them to me, so I can update the asset presets and make them available to all users.", MessageType.Info);
			}
